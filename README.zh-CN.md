...
- **弹出"保护已失效"提示**：负责自动恢复的看门狗计划任务被禁用或删除了。可在 Windows 任务计划程序里重新启用名为 `Kynoptic Watchdog <指纹>` 的任务（名字以安装目录的 8 位指纹结尾，如 `Kynoptic Watchdog 46516965`），或重新安装 Kynoptic。
